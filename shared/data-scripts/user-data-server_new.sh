...
# Runs server.sh that configures and starts Consul, Nomad, and Vault
sudo bash /ops/shared/scripts/server.sh "${cloud_env}" "${server_count}" '${retry_join}' "${nomad_binary}"


# Defines variables

## todo change configuration directory path to /ops/shared/conf
...
# Configure Nomad with Consul ACL token
...
## todo Remove bootstrap and substitute it with initial management token generated from TF
...
## todo create policies and tokens using TF
...
## todo remove when tokens are generated using TF
